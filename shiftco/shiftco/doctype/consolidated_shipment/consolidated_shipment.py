# Copyright (c) 2024, Shiftco and contributors
...
class ConsolidatedShipment(Document):
		...
		if self.shipment:
			for ship in self.shipment:
				if not ship.shipment_id :
					shipment_docname = create_Shiftco_shipment(self,ship)
					if shipment_docname:
	...
	def before_submit(self):
		error_count = 0
		error_message = "Need to Submit Shiftco Shipment "
		if self.shipment:
			for ship in self.shipment:
				if ship.shipment_id :
					if frappe.db.exists("Shiftco Shipment",{"name":ship.shipment_id,"docstatus":0}):
						if error_count > 0 :
	...
	def before_cancel(self):
		if self.shipment:
			for ship in self.shipment:
				if ship.shipment_id :
					shipment_doc = frappe.get_doc("Shiftco Shipment",ship.shipment_id)
	...
	if self.docstatus == 0 :
		...
		if old_shipment_list:
			for docname in old_shipment_list:
				frappe.delete_doc("Shiftco Shipment",docname)



@frappe.whitelist()
def create_Shiftco_shipment(self,ship):
	Shiftco_shipment_doc = frappe.new_doc("Shiftco Shipment")
	Shiftco_shipment_doc.shipping_type = "Ocean Import"
	Shiftco_shipment_doc.consolidated_shipment_ref = self.name
	if ship.consignee:
		Shiftco_shipment_doc.oversea_agent = ship.consignee
	if ship.consignor:
		Shiftco_shipment_doc.forwarding_agent = ship.consignor
	if ship.origin:
		Shiftco_shipment_doc.port_of_loading = ship.origin
	if ship.destination:
		Shiftco_shipment_doc.port_of_discharge = ship.destination
	if ship.etd:
		Shiftco_shipment_doc.etd = ship.etd
	if ship.eta:
		Shiftco_shipment_doc.place_of_delivery_eta = ship.eta
	if ship.house_bill:
		row = Shiftco_shipment_doc.append("hbl",{})
		row.hbl_no =ship.house_bill
	Shiftco_shipment_doc.insert(
	...
	frappe.db.commit()
	return Shiftco_shipment_doc.name
