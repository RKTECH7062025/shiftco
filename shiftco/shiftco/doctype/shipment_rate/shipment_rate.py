...
class ShipmentRate(Document):
	def before_submit(self):
		if self.reference_doctype and self.reference_name:
			...
			if self.reference_doctype == "Shiftco Shipment":
...
def OAuth2_authentication(doc=None,doctype=None,country=None,mode=None):
	...
	oauth2_url = "https://api.Shiftco.com/oauth2/token"
	payload = {
		'username': 'test40@Shiftco.com',
		'password': 'Asfgh@%4!23',
		'grant_type': 'password',
		'scope': ''
	}
	...
	try :
		...
		if oauth2_response.status_code == 200:
			...
			if self.doctype in ["Shiftco Shipment"]:
				final_tracking_data = frappe._dict({})
				for track in self.container:
					track = frappe._dict(track)
					making_Shiftco_shipment(self,track,final_tracking_data,bearer)
...
@frappe.whitelist()
def making_Shiftco_shipment(self,track,final_tracking_data,bearer):
...
def making_shipment_rate(final_list,data,final_data):
	...
	final_obj["rate_id"]=data["ShiftcoId"]
...
def get_all_documents(bearer,doctype,country=None,mode=None):
	...
	if doctype == "Carrier":
		api_url = "https://api.Shiftco.com/v1/carriers"
		params = {
			'isTrackingAvailable': "null",
		}
	if doctype == "Container Type":
		api_url = "https://api.Shiftco.com/v1/load-types"
		params = {}
	
	if doctype == "Port":
		api_url = "https://api.Shiftco.com/v1/sea-ports"
			...
			for item in data:
				...
				if doctype == "Port":
					if not frappe.db.exists("Port",{"code":item["unLoCode"]}) and item["unLoCode"]:
						try:
							frappe.enqueue("Shiftco.Shiftco.doctype.shipment_rate.shipment_rate.create_port_document",queue="long",param=item)
						except Exception as e:
...
def get_price(bearer,self):
	# Define the URL and parameters
	price_url = "https://api.Shiftco.com/v3/prices"
	params = {
		'origin': self.origin,
...
def get_tracking_data(bearer,self,containerId,sealine):
	# Define the URL and parameters
	tracking_status_url = f"https://api.Shiftco.com/v1/track/container/{containerId}"
	params = {
		'sealine': sealine,
	}
	# Define the Bearer token and additional headers
...
def get_schedules(bearer,self):
	url = "https://api.Shiftco.com/v1/schedules"
	params = {
		'origin': self.origin,
		'destination': self.destination,
		'departureDate': self.departure_date,
