            ...
            setTimeout(() => {
                frm.add_custom_button(("Request for Quotation"), function(){
                    frappe.model.open_mapped_doc({
                        method:"Shiftco.Shiftco.doctype.shipment_rate.shipment_rate.make_quotation_sales_order",
                        frm:frm,
                        args:{
                ...
                frm.add_custom_button(("Shipment"), function(){
                    if(!frm.doc.reference_doctype){
                        check_existing(frm,doctype="Shiftco Shipment")
                    }
                    else if(frm.doc.reference_doctype != "Shiftco Shipment"){
                        check_existing(frm,doctype="Shiftco Shipment")
                    }
                    else{
...
function check_existing(frm,doctype){
    frm.call({
        method: "Shiftco.Shiftco.doctype.shipment_rate.shipment_rate.check_shedule_and_rate",
        args: {
            doc:frm.doc,
            doctype:doctype
        },
        ...
        if(r.message.function=="Success"){
            frappe.model.open_mapped_doc({
                method:"Shiftco.Shiftco.doctype.shipment_rate.shipment_rate.make_shipment",
                frm:frm,
                args:{
			...
			if(item.__checked==1){
                ...
                if(function_name=="fetching"){
                    if(JSON.parse(item.charges_json).length >0){
                        ...
                        frm.call({
                            method: "Shiftco.Shiftco.doctype.shipment_rate.shipment_rate.consolidate_item",
                            args: {
                                item:item
                            },
                            freeze:true,
